#Part7. 최단 경로
#문제 1. 1번부터 N번까지의 회사가 있고 M개의 연결된 도로가 있다. 도로는 양방향으로 이동할 수 있으며 모든 도로는 1만큼 시간이 걸린다
#        1번부터 시작하여 K번 회사를 거쳐 X번 회사로 가는 최소 시간을 구하기
...
for a,b in list_a:   #양방향 간선으로 값 넣기
    graph[a][b] = 1
    graph[b][a] = 1    
...
if graph[1][K] + graph[K][X] >= INF:   #갈 수 없는 경우
    ...
    print(graph[1][K]) + print(graph[K][X])  #더해주면 됨 
